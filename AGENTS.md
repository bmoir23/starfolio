...
- Sanity studio lives in `bmoir-dev-portfolio/` (project `cv6an3n5`, dataset `portoflio-dev-blog`). Canonical schemas + seed data: `sanity/schemas/` and `sanity/seed/`. Local env: copy `.env.example` → `.dev.vars` with `SANITY_PROJECT_ID=cv6an3n5` and `SANITY_DATASET=portoflio-dev-blog`. Seed content: `cd bmoir-dev-portfolio && pnpm seed` (requires `sanity login`) or `SANITY_API_TOKEN=... pnpm seed:sanity` from repo root.
